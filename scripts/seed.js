#!/usr/bin/env node
/**
 * Seed script — populates curriculum_versions and prompt_templates with the
 * content currently hardcoded in lib/curriculum.ts and lib/ai-providers.ts.
 *
 * Usage:
...
// ---------------------------------------------------------------------------
// Curriculum content (extracted from lib/curriculum.ts)
// ---------------------------------------------------------------------------
...
const CURRICULUM = require('../lib/curriculum-data.json');

// ---------------------------------------------------------------------------
// Prompt templates (extracted from lib/ai-providers.ts)
// ---------------------------------------------------------------------------
...
const MODEL_OPTIONS = JSON.stringify([
  {
    id: 'anthropic',
    name: 'Claude Sonnet',
...
async function main() {
  ...
  try {
    // ── Seed curriculum ────────────────────────────────────────────────────
    console.log('Seeding curriculum_versions…');

    // Deactivate any existing active version
    await client.query(`UPDATE curriculum_versions SET is_active = false WHERE is_active = true`);

    await client.query(
      `INSERT INTO curriculum_versions (label, content, is_active)
       VALUES ($1, $2, true)`,
      ['v1-seed', JSON.stringify(CURRICULUM)]
    );
    console.log('  ✓ Curriculum v1 inserted and activated.');

    // ── Seed prompt templates (system defaults: user_id = NULL) ─────────
    console.log('Seeding prompt_templates…');

    const templates = [
      { slug: 'lesson',  title: 'Lesson System Prompt',  content: LESSON_SYSTEM_PROMPT },
      { slug: 'ask',     title: 'Ask AI System Prompt',  content: ASK_SYSTEM_TEMPLATE },
      { slug: 'diagram', title: 'Diagram System Prompt', content: DIAGRAM_SYSTEM_PROMPT },
      { slug: 'models',  title: 'Model Catalog (JSON)',  content: MODEL_OPTIONS },
    ];
